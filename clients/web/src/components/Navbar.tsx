    ...
    <nav className="h-14 bg-background border-b border-border sticky top-0 z-50">
      <div className="px-4 sm:px-6 lg:px-8 h-full flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          ...
          {showLoggedIn && (
            <div className="relative ml-1 sm:ml-2" data-user-menu>
              <button
                onClick={() => user && setMenuOpen(!menuOpen)}
                className="flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1.5 rounded-lg text-sm font-medium text-muted hover:text-foreground hover:bg-card transition-colors"
              >
          ...
          {/* Mobile login link (when not logged in) */}
          {!showLoggedIn && !isLoading && (
            <Link
              href="/login"
              className="md:hidden ml-1 px-3 py-1.5 rounded-lg text-xs font-semibold bg-accent text-white hover:bg-accent-hover transition-colors"
            >
              Prijava
            </Link>
          )}
