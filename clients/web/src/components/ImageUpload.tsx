...
export function ImageUpload({ onUpload, isLoading }: ImageUploadProps) {
  ...
  const addFiles = useCallback((newFiles: File[]) => {
    const validFiles = newFiles.filter((f) => ALLOWED_TYPES.has(f.type) || f.type.startsWith("image/"));
    setFiles((prev) => {
      ...
      >
        <label className="flex flex-col items-center justify-center p-8 cursor-pointer">
          ...
          <p className="text-xs text-muted">
            Do {MAX_IMAGES} datoteka &middot; JPG, PNG, WebP, HEIC, PDF, Word, Excel
          </p>
        </label>
      </div>
              ...
              <div key={i} className="relative group aspect-square rounded-lg overflow-hidden border border-border">
                {src ? (
                  ...
                  <div className="w-full h-full bg-card flex flex-col items-center justify-center text-muted">
                    ...
                    <span className="text-[10px] font-medium uppercase">{files[i]?.name.split('.').pop()}</span>
                  </div>
                )}
                {!isLoading && (
                  <button
